...
// 记录开始运行时间
$GLOBALS['_beginTime'] = microtime(TRUE);// 返回当前 Unix 时间戳和微秒数,当
// 记录内存初始使用
define('MEMORY_LIMIT_ON',function_exists('memory_get_usage'));
if(MEMORY_LIMIT_ON) $GLOBALS['_startUseMems'] = memory_get_usage();//返回分配给 PHP 的内存量
// 版本信息
const THINK_VERSION     =   '3.2.3';//tp版本号
...
defined('COMMON_PATH')  or define('COMMON_PATH',    APP_PATH.'Common/'); // 应用公共目录 app下面的Common
...
单引号（’）、双引号（”）、反斜线（）与 NUL（NULL 字符）等字符都会被加上反斜线。这些转义是必须的，如果这个选项为off，
那么我们就必须调用addslashes这个函数来为字符串增加转义。
在php5.4以后就废除了此特性。所以我们在以后就不要依靠这个特性了。
为了使自己的程序不管服务器是什么设置都能正常执行。可以在程序开始用get_magic_quotes_runtime检测该设置的状态决定是否要手工处理，
...
if(!IS_CLI) { //如果不是命令行模式的话，指定当前运行脚本的文件名。
    // 当前文件名
...
// 加载核心Think类
require CORE_PATH.'Think'.EXT; //加载 Think.class.php
// 应用初始化
Think\Think::start();