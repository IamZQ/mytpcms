...
function C($name=null, $value=null,$default=null) {
    ...
    // 无参数时获取所有
    if (empty($name)) {//未传入任何参数则返回全部配置信息
        return $_config;
    }
    // 优先执行设置获取或赋值
    if (is_string($name)) { //传入字符串
        if (!strpos($name, '.')) {
            $name = strtoupper($name);
            if (is_null($value))
                return isset($_config[$name]) ? $_config[$name] : $default;
            $_config[$name] = $value;//值不为NULL的时候设置config return NULL
            return null;
        }
        // 二维数组设置和获取支持
        $name = explode('.', $name);//转换成数组
...
function load_config($file,$parse=CONF_PARSE){
    ...
    switch($ext){
        ...
        case 'json':
            return json_decode(file_get_contents($file), true);//转换成数组
        default:
...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    switch(strtolower($method)) {
        ...
        case 'put'     :   
        	...
        	break;
        case 'param'   :
            switch($_SERVER['REQUEST_METHOD']) { //表单提交的方式
                case 'POST':
                    $input  =  $_POST;
                    break;
                case 'PUT':
    ...
    if(''==$name) { // 获取全部变量
        ...
        if($filters) {
            if(is_string($filters)){
                $filters    =   explode(',',$filters);//用逗号分隔过来参数
            }
...
function array_map_guo_lv($filter,$data){
    $result = array();
    foreach($data as $k=>$value){
        $result[$k] = is_array($value) ? array_map_guo_lv($filter,$value) : call_user_func($filter,$value);
    }
    return $result;
}
...
function load_file($path){
    if($files = C('LOAD_EXT_FILE')){//加载自定义外部文件,可能是一个或者是多个，多个的话以英文逗号隔开
        $files = explode(',',$files);//分隔成为数组
        foreach($files as $val){
            $file = $path."Common/".$val.'.php';//加上路径
            if(is_file($file)) include $file;//判断是否存在此文件，存在的话就加载进来
        }
    }
    //加载定义的动态配置文件
    if($configs  = C("LOAD_EXT_CONFIG")){
...
//不区分大小写的in_array()实现，全部都转换成小写来比较，array_map()将回调函数作用到给定数组的单元上,其中的回调函数是 strtolower(  )
function in_array_ohter_case($value,$array){
    return in_array(strtolower($value),array_map('strtolower',$array));
}
